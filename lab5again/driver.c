...
void generateCFG(list_t* vertex_list, int maxsucc, Random* r){
    ...
    list_t* tmp_list_s;
	//Connect vertex[0] with vertex[1]
    tmp_v = tmp_list->next->data;
	connect(tmp_list->data, tmp_v);
	//Connect vertex[0] with vertex[2]
...
void* spu_init(void *arg)
{
	...
	printf("PPU sees SPU[%u] has terminated.\n", data->spu_num);
...
int main(int ac, char** av){
	...
	for (int i = 0; i < nspu; ++i) {
		printf("Setting SPU params for %d\n", i);
		data[i].spu_num = param[i].proc = i;
        printf("PPU param.proc : %d\n", param[i].proc);
